...
def irb_require(gemname, lib = nil)
  ...
  raise LoadError, "could not load #{lib || gemname} via irb_require" if candidates.empty?

  puts "manipulating load path for #{gemname}"
  $LOAD_PATH.push("#{candidates.sort.reverse.first}/lib")
  require lib || gemname
end
...
def clear
  system 'clear'
  if ENV['RAILS_ENV']
    'Rails environment: ' + ENV['RAILS_ENV']
  else
...
def fl(file_name)
  file_name += '.rb' unless /\.rb/.match?(file_name)
  @@recent = file_name
  load "#{file_name}"
end
